...
toMASM checkImports m = do
  ...
  where checkImps xs
        ...
        callGraph = Map.unionsWith (<>)
          ...
          | (caller, (_mname, Left (W.Function _ _ instrs))) <- Map.toList allFunctionsMap
          , W.Call callee <- instrs
          ]
        ...
        sortedFuns =
          let xs = enumerate mainFunId
              rxs = reverse xs
              go [] _ = []
              go (a:as) !visited
                | a `Set.member` visited = go as visited
                | otherwise = case Map.lookup a allFunctionsMap of
                    Just (mname, Left f) -> (a, mname, f) : go as (Set.insert a visited)
                    _ -> go as (Set.insert a visited)
          in go rxs Set.empty
        ...
        translateInstr _ W.Drop = withPrefix $ \ty ->
